...
there. The one compatibility path belongs only to inline `rename`: rclone 1.75 returns `EINVAL` for
`RENAME_NOREPLACE` on a directory under a mount identified exactly as `fuse.rclone` in
`/proc/self/mountinfo`. Ordinary rclone directory rename is never used because it was proven to replace
even a non-empty target. `ops::rename_path` instead builds the target through the existing exclusive
copy primitives, removes the source only after the copy completes, and uses the same path for undo.
A copy failure removes only the partial target this operation created; a source-removal failure keeps the
complete target copy and reports the error. Files, other filesystems and every other `rename_noreplace`
caller stay on the atomic syscall. Mountinfo's escaped mount point is decoded and the deepest enclosing
mount wins, so a nested non-rclone mount cannot inherit the exception.
