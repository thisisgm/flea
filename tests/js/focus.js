...
function run(check) {
    ...
    // Finder's Cmd+1/2/3: the same Pane.setView the chrome's three buttons call, so the chord and
    // the click persist the one stored view. The mock records what reaches it, because a chord
    // that switched the view on screen without calling setView would otherwise pass this suite.
    var viewed = listPane(true)
    viewed.persisted = []
    viewed.setView = function (mode) { this.viewMode = mode; this.persisted.push(mode) }
    ...
    check("ctrl 3, 2 and 1 pick the grid, the columns and the list, through setView each time",
