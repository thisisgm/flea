...
function run(check) {
    ...
    check("the state verbs land on the state", state.calls.join(","),
          'cancel,stopFetch,mark 4,activate 4,parent,back,hidden,selectAll,send {"c":"trash","paths":["/d/a.txt"]},undo')
    ...
    check("the state is untouched by either", state.calls.length, 10)
