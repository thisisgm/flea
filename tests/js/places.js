...
function run(check) {
    ...
    check("Trash is second", favs[1].path + "|" + favs[1].label, "/home/gm/.local/share/Trash/files|Trash")
    check("the XDG dirs follow in file order", favs[2].label + "," + favs[3].label, "Downloads,Documents")
    check("a path already placed keeps its first position and its first label",
          favs.map(function (e) { return e.label }).join(","), "Home,Trash,Downloads,Documents,Media")
    ...
    check("the mark is resolved by the caller, so the rail keeps its own Icons import", favs[2].glyph, "mark:Downloads")
