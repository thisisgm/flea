...
# Usage: ./tests/picker.sh [pick|selectall|copypaste|click|menu|duplicate|trash|crumbs|rail|save|savename|typed|typed_dir|typed_file|typed_url|typed_share|cancel|withdrawn|died|fault|pills|filter|header|hidden|thumbs|taildrop|dragout]; typed_share, taildrop and dragout are opt-in.
...
backend_is_flea
[[ "$#" -gt 0 ]] || set -- pick selectall copypaste click menu duplicate trash crumbs rail save savename typed typed_dir typed_file typed_url cancel withdrawn died fault pills filter header hidden thumbs
for name in "$@"; do
    case "$name" in
        pick) case_pick ;;
