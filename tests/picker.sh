...
# Usage: ./tests/picker.sh [pick|selectall|copypaste|click|menu|crumbs|rail|save|savename|typed|typed_dir|typed_file|typed_url|typed_share|cancel|withdrawn|died|fault|pills|filter|header|hidden|thumbs|taildrop|dragout]; typed_share, taildrop and dragout are opt-in.
...
case_menu() {
    ...
    [[ "$hints" == "enter|||Ctrl+X|Ctrl+C|Ctrl+V||r||d||||." ]] \
        || fail "menu: the row menu hints are $hints"
...
backend_is_flea
[[ "$#" -gt 0 ]] || set -- pick selectall copypaste click menu crumbs rail save savename typed typed_dir typed_file typed_url cancel withdrawn died fault pills filter header hidden thumbs
for name in "$@"; do
    case "$name" in
        pick) case_pick ;;
