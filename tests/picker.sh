...
# Usage: ./tests/picker.sh [pick|save|savename|typed|typed_dir|typed_file|cancel|withdrawn|died|fault|taildrop]; taildrop is opt-in.
...
backend_is_flea
[[ "$#" -gt 0 ]] || set -- pick save savename typed typed_dir typed_file cancel withdrawn died fault
for name in "$@"; do
    case "$name" in
        pick) case_pick ;;
        save) case_save ;;
