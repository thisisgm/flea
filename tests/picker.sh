...
# Usage: ./tests/picker.sh [pick|click|crumbs|save|savename|typed|typed_dir|typed_file|typed_url|typed_share|cancel|withdrawn|died|fault|taildrop]; typed_share and taildrop are opt-in.
...
backend_is_flea
[[ "$#" -gt 0 ]] || set -- pick click crumbs save savename typed typed_dir typed_file typed_url cancel withdrawn died fault
for name in "$@"; do
    case "$name" in
        pick) case_pick ;;
        click) case_click ;;
