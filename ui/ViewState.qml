...
QtObject {
    ...
    // The view's one writer: ui/Pane.qml's setView routes both the chrome's buttons and the
    // ctrl-1/2/3 chords here, so the two entrances cannot persist two different answers. owe()
    // sends nothing for a value the window already holds, so a repeated chord is not a patch.
